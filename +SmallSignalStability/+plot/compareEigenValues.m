...
for i = 1:10
    [~,~,p] = damp_(A_matrices{i});
    sh{i} = scatter(real(p),imag(p),'Visible','off','CData',cmp(i,:)); % TODO: use eigPlot
end
sh{1}.Visible = 'on';
legend([sh{:}],string(1:10),'Location','best','NumColumns',2)
ah.XLim = [-1200 200];
ah.YLim = [-15 15];
